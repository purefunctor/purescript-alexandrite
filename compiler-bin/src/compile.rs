...
fn select_package_sources(
            ...
            if let Some(first_package) = owners.get(&source) {
                if first_package != &package.name {
                    return Err(CompileError::ConflictingPackageSource {
                        path: source,
                        first_package: first_package.clone(),
                        second_package: package.name.clone(),
                    });
                }
            } else {
                owners.insert(source, package.name.clone());
            }
        }
    }
    ...
    for source in source_paths {
        ...
        selected.entry(owner.clone()).or_default().push(source.to_path_buf());
    }
...
fn package_groups(jobs: &[PackageJob]) -> Vec<PackageGroup> {
    ...
    let mut dependents = vec![Vec::new(); group_packages.len()];
...
mod tests {
    ...
    fn package(name: &str, dependencies: Vec<usize>) -> PackageJob {
        PackageJob { name: name.to_owned(), source_ids: Vec::new(), dependencies }
    }
    ...
    fn package_source_ownership_must_be_unique() {
        ...
        let source_paths = BTreeSet::from([source.clone()]);
        let packages = vec![
            PackageInput {
                name: "first".to_owned(),
    ...
    fn source_empty_packages_preserve_dependency_reachability() {
        ...
        let source_paths = BTreeSet::from([library.clone(), application.clone()]);
        let packages = vec![
            PackageInput {
        ...
        let worker = thread::spawn(move || {
            ...
            pool.install(|| {
                schedule_package_jobs(&jobs, &progress, &|package| {
                    started.send(package.name.clone()).unwrap();
    ...
    fn packages_are_scheduled_exactly_once() {
        ...
        schedule_package_jobs(&jobs, &progress, &|package| {
            ...
            *executions.entry(package.name.clone()).or_insert(0) += 1;
            Ok(())
        })
        .unwrap();
