...
pub(crate) fn build(config: BuildConfig<'_>) -> Result<(), CompileError> {
    ...
    for path in source_paths {
        sources.insert(path.clone(), load_source(&mut compilation, &path)?);
    }
...
fn write_modules(
    ...
    if modules.iter().any(|module| module.requires_runtime()) {
        ...
        owned_outputs.insert(runtime.clone());
...
fn write_module(
    ...
    .and_then(|()| write_if_changed(&output_path, module.source().as_bytes()));
