    ...
    pub fn new(
        ...
        for package in &package_inputs {
            if !package_names.insert(package.name.clone()) {
                return Err(BuildPlanError::DuplicatePackage(package.name.clone()));
            }
                ...
                if let Some(first_package) = owners.get(source_identity) {
                    if first_package != &package.name {
                        return Err(BuildPlanError::ConflictingPackageSource {
                            path: source_identity.clone(),
                            first_package: first_package.clone(),
                            second_package: package.name.clone(),
                        });
                    }
                } else {
                    owners.insert(source_identity.clone(), package.name.clone());
                }
            }
        }
        ...
        for source in selected_sources {
            ...
            selected_by_package.entry(owner.clone()).or_default().push(source.path);
        }
