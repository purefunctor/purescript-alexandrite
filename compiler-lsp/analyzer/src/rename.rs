...
pub fn implementation(
    ...
    for location in locations {
        let file_id =
            context.files.file_id(location.uri.as_str()).ok_or(AnalyzerError::NonFatal)?;
        if !editable_file(context, workspace_root, file_id) {
            continue;
        }

        let range = reference_name_range(context, file_id, location.range, name_kind)?;
        let new_name = new_name_text(context, file_id, range, &new_name)?;
        edits.push((file_id, TextEdit { range, new_text: new_name }));
    }
...
fn qualifier_edits(
    ...
    for qualified in qualified_names {
        ...
        push_text_range_edit(context, file_id, token.text_range(), &new_name, edits)?;
    }
