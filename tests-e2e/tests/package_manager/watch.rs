...
impl WatchProcess {
    ...
    fn stdout(&self) -> String {
        self.stdout.lock().expect("invariant violated: watch stdout capture is poisoned").clone()
    }

    fn stderr(&self) -> String {
        self.stderr.lock().expect("invariant violated: watch stderr capture is poisoned").clone()
    }
