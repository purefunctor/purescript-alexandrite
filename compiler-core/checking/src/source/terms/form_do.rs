    ...
    Q: ExternalQueries,
{
    ...
    let applications =
        unification::subtype_with_applications(state, context, expression.type_id, argument)?;
    let expression = unification::materialize_subtype_applications(state, expression, applications);
